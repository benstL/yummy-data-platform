...
# Use singular forms (tomato / strawberry / potato) so test assertions on
# exact EUFIC token matching are reliable; plurals give FAOSTAT matches.
...
def generate_recipes() -> pd.DataFrame:
    """Generate 50 fake Silver recipe rows covering all INGREDIENT_TEMPLATES."""
    ...
    for i in range(1, N_RECIPES + 1):
        ...
        rows.append({
            ...
            "recipecategory": ["main", "dessert", "side", "breakfast"][(i - 1) % 4],
...
def generate_reviews() -> pd.DataFrame:
    """Generate 200 fake Silver review rows (4 per recipe: 2 positive, 1 neutral, 1 negative)."""
    rng = np.random.default_rng(SEED)
    now = datetime.now(UTC).isoformat()
    rows = []
    review_id = 1
...
def generate_seasonality() -> pd.DataFrame:
    """Generate fake Silver EUFIC seasonality rows for 5 key products, country 'fr'."""
    # (product_name, product_type, in-season month numbers)
    products = [
...
def generate_faostat() -> pd.DataFrame:
    """Generate fake Silver FAOSTAT production rows for 5 key products, country 'france'."""
    now = datetime.now(UTC).isoformat()
    # Deliberately use plural / qualified names to create fuzzy-match test cases
    rows = [
        (68, "france", 388, "tomatoes",           "production", 2022, "tonnes",  850_000.0),
        (68, "france", 393, "strawberries",        "production", 2022, "tonnes",   60_000.0),
        (68, "france", 116, "potatoes",            "production", 2022, "tonnes", 5_400_000.0),
        (68, "france", 137, "carrots and turnips", "production", 2022, "tonnes",  500_000.0),
        (68, "france", 406, "garlic (dry)",        "production", 2022, "tonnes",   18_000.0),
    ]
    df = pd.DataFrame(rows, columns=[
        "area_code", "country_name", "item_code", "product_name",
        "element", "year", "unit", "production_value",
    ])
